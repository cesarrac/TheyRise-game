...
public class EnemyTask
{
    ...
    public EnemyTask(string name, float genPriority)
    {
        taskName = name;
        GeneralPriority = genPriority;
    }

    public float CalcAssignment (float modifier, float maxTasks, float distance)
    {
        return (maxTasks - (this.generalPriority + modifier)) / distance;
    }
}
...
public class Enemy_Master : MonoBehaviour {
    ...
    void InitTasks()
    {
        tasks = new EnemyTask[] { new EnemyTask("Kill Player", 1), new EnemyTask("Destroy Utility Tower", 2), new EnemyTask("Destroy Battle Tower", 3) };
    }
    ...
    // This gets called by the Resource Grid every time the player builds a Battle Tower (Machine Gun, Sniper, etc.)
    ...
    void SortTasks()
    {
        ...
        if (battleTowersBuilt.Count != battleTowerCount)
        {
            ...
            if (battleTowerCount % 3 == 0)
            {
                // ... if it's true then the result of the division is the modifier.
        ...
        {
            tasks[1].CalcAssignment(battleModifier, 3, (nearestBattleTower.position - spawnPosition).magnitude);
        }
        else
        {
            tasks[1].CalcAssignment(battleModifier, 3, 100f);
        }

        // Now all tasks must be sorted by their Assignment Score
        // TODO: Use a Sort interface to sort the scores by lowest to highest (lowest being the higher priority task)
    }
    ...
    // First Action, Figure out a Target for the next wave
    void SelectATarget()
    {
    ...
    //        Enemy_Spawner.instance.ReceiveSpawnCommand(total, e);
    //    }
    //    else
    //    {
